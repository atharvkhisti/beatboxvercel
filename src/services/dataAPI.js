...
export async function getlyricsData(id) {
  try {
    const response = await fetch(`${SAAVN_API_BASE}/api/songs/${id}/lyrics`);
    const data = await response.json();
    return data;
  } catch (error) {
    console.log(error);
...
export async function getSearchedData(query) {
  try {
    const response = await fetch(`${SAAVN_API_BASE}/api/search?query=${query}`);
    const data = await response.json();
    return data?.data;
  } catch (error) {
    console.log(error);
...
// AutoMix helper: call backend API to generate a mix
export async function getAutoMix(mood, activity) {
  try {
    const response = await fetch("/api/autoMix", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    console.log("AutoMix API error", error);
  }
}
